...
const ALL_COLUMNS = [
  // { id: 'portfolioName', label: 'Portfolio', align: 'left' },
  { id: 'code', label: 'Code', align: 'left', parent: 'userInput' },
  { id: 'address', label: 'Address', align: 'left', parent: 'userInput' },
  { id: 'occupancy', label: 'Occupancy', align: 'left', parent: 'userInput' },
  { id: 'premium', label: 'Premium', align: 'right', parent: 'userInput' },

  { id: 'latitude', label: 'Latitude', align: 'right', parent: 'Info' },
  { id: 'longitude', label: 'Longitude', align: 'right', parent: 'Info' },
  { id: 'elevation', label: 'Elevation', align: 'right', parent: 'Info' },
  { id: 'coniferous_forest_proximity', label: 'Forest Proximity', align: 'right', parent: 'Info' },
  { id: 'coniferous_forest_type', label: 'Forest Type', align: 'left', parent: 'Info' },
  { id: 'body_of_water_proximity', label: 'Water Proximity', align: 'right', parent: 'Info' },
  { id: 'fault_line_proximity', label: 'Fault Line Proximity', align: 'right', parent: 'Info' },

  { id: 'wildfire_status', label: 'Wildfire Status', align: 'left', parent: 'hazard' },
  { id: 'wildfire_risk_value', label: 'Wildfire Risk', align: 'right', parent: 'hazard' },
  { id: 'flood_risk_areas', label: 'Flood Risk', align: 'left', parent: 'hazard' },
  { id: 'flood_status', label: 'Flood Status', align: 'left', parent: 'hazard' },
  { id: 'hail_alley_proximity', label: 'Hail Proximity', align: 'right', parent: 'hazard' },

  { id: 'resComm', label: 'Res/Comm', align: 'left', parent: 'customParent' },
  { id: 'constructionType', label: 'Construction Type', align: 'left', parent: 'customParent' },
  { id: 'valueAtRisk', label: 'Value at Risk', align: 'right', parent: 'customParent' },
  { id: 'street_address', label: 'Street Address', align: 'left', parent: 'customParent' },
  { id: 'city', label: 'City', align: 'left', parent: 'customParent' },
  { id: 'state', label: 'State', align: 'left', parent: 'customParent' },
  { id: 'postal_code', label: 'Postal Code', align: 'left', parent: 'customParent' },
  { id: 'country', label: 'Country', align: 'left', parent: 'customParent' },
  { id: 'deductible', label: 'Deductible', align: 'right', parent: 'customParent' },
  { id: 'limit', label: 'Limit', align: 'right', parent: 'customParent' },
  { id: 'attachment', label: 'Attachment', align: 'right', parent: 'customParent' },
  { id: 'participation', label: 'Participation', align: 'right', parent: 'customParent' },
  { id: 'basement', label: 'Basement', align: 'center', parent: 'customParent' },
  { id: 'excess', label: 'Excess', align: 'right', parent: 'customParent' },
  { id: 'ecoregion_name', label: 'Ecoregion', align: 'left', parent: 'customParent' },
  { id: 'drought_condition', label: 'Drought', align: 'left', parent: 'customParent' },
  {
  ...
  { id: 'earthquake_alert', label: 'Earthquake Alert', align: 'left', parent: 'customParent' },
  { id: 'magnitude', label: 'Magnitude', align: 'right', parent: 'customParent' },
];

const PARENT_COLUMN = ['userInput', 'Info', 'hazard', 'customParent'];
...
export default function PortfolioTable({ data, currentPortfolio, portfolios
  ...
  const handleDeleteRows = useCallback(
    async (selectedIds) => {
      try {
        ...
        await Promise.all(selectedIds.map((id) => axios.delete(`/api/locations/${id}`)));
  ...
  const handleParentToggle = (parent) => {
    const childColumnIds = ALL_COLUMNS.filter((column) => column.parent === parent).map(
      (column) => column.id
    );

    setVisibleColumns((prev) => {
      const isParentSelected = childColumnIds.every((id) => prev.includes(id));
      if (isParentSelected) {
        // Uncheck all children under this parent
        return prev.filter((id) => !childColumnIds.includes(id));
      } else {
        // Check all children under this parent
        return [...prev, ...childColumnIds.filter((id) => !prev.includes(id))];
      }
    });
  };
