...
export default function PortfolioTableRow({ 
  ...
  // const renderDetailValues = (field, value) => {
  ...
  const renderDetailValue = (field, value, editableFields, setEditableFields) => {
    const isEditing = editableFields[field.id];
    ...
    const handleSaveEdit = () => {
      console.log(`Save ${field.id}: ${fieldValue}`);
      setEditableFields((prev) => ({
        ...prev,
        [field.id]: false,
      }));
      // Optionally update the `value` at the parent level with a callback if needed
    };
                  ...
                  {group.fields.map((field) => (
                    <Grid item xs={12} sm={6} md={4} key={field.id}>
                      <Stack spacing={0.5}>
                        ...
                        {renderDetailValue(field, row[field.id], editableFields, setEditableFields)}
                      </Stack>
                    </Grid>
                  ))}
                </Grid>

                {index !== DETAIL_GROUPS.length - 1 && <Divider sx={{ mt: 3 }} />}
              </Box>
            ))}
          </Box>
        </DialogContent>
